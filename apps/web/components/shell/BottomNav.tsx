...
export function BottomNav() {
  ...
  return (
    <nav
      className="fixed inset-x-0 bottom-0 z-40 border-t border-white/[0.07] bg-graphite-900/80 shadow-bar backdrop-blur-xl"
      ...
      <div className="mx-auto flex max-w-xl items-stretch justify-around px-3">
        {items.map(({ href, label, Icon, match }) => {
          ...
          return (
            ...
            >
              <span className={['grid h-9 w-9 place-items-center rounded-xl border transition', active ? 'border-white/15 bg-white/10' : 'border-transparent'].join(' ')}>
